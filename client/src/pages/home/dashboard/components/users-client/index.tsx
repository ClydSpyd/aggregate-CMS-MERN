...
const debounceInput = debounce(
  (
    input: string,
    setFilteredUsers: Dispatch<SetStateAction<ClientUser[] | null>>,
    users: ClientUser[]
  ) => {
...
export default function UsersClient() {
  ...
  const [filteredUsers, setFilteredUsers] = useState<ClientUser[] | null>(null);
  const [searchInupt, setSearchInput] = useState<string>("");

  useEffect(() => {
    const fetchUsers = async () => {
      const { data, error } = await API.user.getAllClientUsers();
      if (data) {
        delay(500).then(() => {
          setUsers(data);
